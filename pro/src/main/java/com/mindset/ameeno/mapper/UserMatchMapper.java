...
public interface UserMatchMapper extends MyMapper<UserMatch> {
     ...
     //无限聊天
     @Update("update user_match set infinityChat = #{iState} where my_user_id = #{userId} and match_user_id = #{matchUserId};"+
             " update user_match set infinityChat = #{iState} where my_user_id = #{matchUserId} and match_user_id = #{userId};")
     void updateChatTime(String userId, String matchUserId, int cState);
}