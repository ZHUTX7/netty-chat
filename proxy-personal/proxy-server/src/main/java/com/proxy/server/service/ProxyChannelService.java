...
public class ProxyChannelService implements LifeCycle {
     ...
     * @throws Exception
     */
    public void bindForUDP(Integer serverPort, Bootstrap bootstrap, ProxyRealServer proxyRealServer) throws Exception {
        proxyChannelDao.bindForUDP(serverPort, bootstrap, proxyRealServer);
    }
    ...
    public void shutDown() {
        ...
        try {
            ...
            for (Map.Entry<Object, ProxyChannel> entry : map.entrySet()) {
                ...
                entry.getValue().getClientKey();
                logger.debug("代理服务(端口/域名):{} 退出", entry.getKey());
            }
        } catch (Exception ignored) {
            logger.debug("代理服务退出失败");
        }
    }
}
