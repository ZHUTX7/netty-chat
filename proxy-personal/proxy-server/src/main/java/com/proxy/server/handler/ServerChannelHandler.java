...
public class ServerChannelHandler extends ChannelInboundHandlerAdapter {
    ...
    private void handleTransferMessage(ChannelHandlerContext ctx, ProxyMessage proxyMessage) {
        ...
        logger.debug(" 转发的session iD 为 : "+ sessionId);
        Channel userChannel = ServerBeanManager.getUserSessionService().get(sessionId);
        if (userChannel != null) {
            //直接使用堆外内存，减少一次用户态数据到内核态的拷贝
            ByteBuf buf = ctx.alloc().directBuffer(proxyMessage.getData().length);
            buf.writeBytes(proxyMessage.getData());
            userChannel.writeAndFlush(buf);
            logger.info("代理服务器将数据转发给用户");
        } else {
