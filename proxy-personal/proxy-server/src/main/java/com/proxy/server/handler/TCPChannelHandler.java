...
public class TCPChannelHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        // TODO: 待修改， 日志写入应该放后台执行，不影响TCP转发性能
       // jedis.getResource().zincrby(RedisKeyNameConfig.getUserConnectConfig(),1,sa.getAddress().getHostAddress());


        logger.debug("用户请求访问通道设置为不可读,等待代理服务器与真实服务器建立连接");
    ...
    private void tcpHandler(ChannelHandlerContext ctx, ByteBuf buf, Integer proxyType) throws Exception {
        ...
        AttributeKey<String> attributeKey = AttributeKey.valueOf("key1");//参数key
        ctx.channel().attr(attributeKey).set("用户流量");
        Channel userChannel = ctx.channel();
